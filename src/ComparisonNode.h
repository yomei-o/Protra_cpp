...
#ifndef DF_CONPARINODE_H_
#define DF_CONPARINODE_H_
...
class ComparisonNode : public ExpressionNode
{
    ...
    std::vector<std::string> _opStrList;

    std::shared_ptr<ExpressionNode> Parse(std::shared_ptr<ExpressionNode>& that, Scanner& scanner)override
    {
        Token = scanner._Token;
        std::shared_ptr<ExpressionNode> node = std::shared_ptr<ExpressionNode>(new ArithmeticExpressionNode());
        ...
        do
        {
            _opStrList.push_back(token);
            scanner.Scan();
            std::shared_ptr<ExpressionNode> tmp = std::shared_ptr<ExpressionNode>(new ArithmeticExpressionNode());
