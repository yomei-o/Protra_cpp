...
#ifndef DF_EQUIVALNNODE_H_
#define DF_EQUIVALNNODE_H_
...
class EquivalenceComparisonNode : public ExpressionNode
{
public:
    ...
    std::shared_ptr<ExpressionNode> Parse(std::shared_ptr<ExpressionNode>& that,Scanner& scanner)override
    {
        ...
        std::shared_ptr<ExpressionNode> node = std::shared_ptr<ExpressionNode>(new ComparisonNode());
        ...
        do
        {
            ...
            scanner.Scan();
            std::shared_ptr<ExpressionNode>tmp = std::shared_ptr<ExpressionNode>(new ComparisonNode());
    ...
    std::shared_ptr<Value> Evaluate(std::shared_ptr<ExpressionNode>& that, Resource& resource, int at, std::string& ats)override
    {
        ...
        for (int i = 1; i < _nodeList.size(); i++)
        {
            ...
            val1 = _opCharList[i - 1] == '=' ?
                std::shared_ptr<Value>(new Value(*val1.get() == *val2.get())) :
                std::shared_ptr<Value>(new Value(*val1.get() != *val2.get()));
        }
        return val1;
        //}
