...
#ifndef DF_MUJINZOUUPDATOR_H_
#define DF_MUJINZOUUPDATOR_H_
...
class MujinzouUpdator:public PriceDataUpdator{
public:
	...
	int Update() override {
		...
		d_now = Now();
		if (Hour() < 17)d_now = Protra::Lib::Data::DateTime(d_now.Value - 1);

		Protra::Lib::Data::DateTime dt_first(2015, 1, 4);
		...
		while (d_maxdate.Value <= d_now.Value) {
			...
			//http://mujinzou.com/k_data/2020/20_03/T200304.zip
			sprintf(buf_url,"http://mujinzou.com/k_data/20%02d/%02d_%02d/T%02d%02d%02d.zip",yy,yy,mm,yy,mm,dd);
			sprintf(buf_file, "T%02d%02d%02d.csv",yy, mm, dd);

			url = buf_url;
			file = buf_file;

			{
				DownloadUtil du(url, file, path_tmp, COMPTYPE_ZIP);
			...
			path_csv = path_tmp + "/" + file;
			{
				...
				while (1) {
					...
					if (entries.size() < 9)continue;
					c = 0; y = 0; m = 0; d = 0;
					d1 = 0; d2 = 0; d3 = 0; d4 = 0; d5 = 0;
					sscanf(entries[0].c_str(), "%d/%d/%d", &y, &m, &d);
					sscanf(entries[1].c_str(), "%d", &c);
					sscanf(entries[4].c_str(), "%lf", &d1);
					sscanf(entries[5].c_str(), "%lf", &d2);
					sscanf(entries[6].c_str(), "%lf", &d3);
					sscanf(entries[7].c_str(), "%lf", &d4);
					sscanf(entries[8].c_str(), "%lf", &d5);
